...
F: When you create a Pokémon and try to access its fields directly – does it work? Why or why not?
        A: Nej det går inte, för fälten är private (inte public), vilket gör att bara är den abstrakta klassen som kan komma åt dem direkt.
          Alla andra klasser måste använda sig av get/set-metoderna för att komma åt dem.
...
F: When you loop through the list and call Attack(), what ensures that the correct attack behavior is executed for each Pokémon?
        A: För att när vi anropar Attack() så används attacker som är medskickade till den specifika pokemonen via konstruktorn, så de är redan medskickade när vi skapar pokemonen. 
